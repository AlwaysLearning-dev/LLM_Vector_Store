...
class Pipeline:
    ...
    def extract_search_terms(self, query: str) -> List[str]:
        # Combine all terms or quoted phrases into a single string for vector search
    ...
    def vector_search_qdrant(self, query_text: str, top_k: int = 10) -> List[Dict]:
        """Use vector similarity search with Qdrant, specifying vector_name='default'."""
        if not query_text:
            return []
        try:
            ...
            results = self.qdrant.search(
                ...
                limit=top_k,
                with_payload=True,
                vector_name="default"  # Ensure this matches ingestion
            )
...
Please be specific and refer to the rules when applicable."""
    ...
    def pipe(self, prompt: str = None, **kwargs) -> Generator[str, None, None]:
        ...
        try:
            # Convert user query to a single text for vector search
