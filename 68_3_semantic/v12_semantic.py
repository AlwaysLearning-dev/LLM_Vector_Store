...
class Pipeline:
    ...
    def __init__(self):
        ...
        # Initialize Qdrant client with URL format
        qdrant_url = f"http://{self.valves.QDRANT_HOST}:{self.valves.QDRANT_PORT}"
    ...
    async def on_startup(self):
        ...
        try:
            print(f"Verifying Qdrant connection...")
            ...
            # Verify embedding model matches ingestion
            test_embedding = self.model.encode(["Dimension test"])[0]
            dimension = len(test_embedding)
            print(f"Embedding model dimension: {dimension}")
            
            # Test LLM connection if enabled
            if self.valves.ENABLE_CONTEXT:
                try:
    ...
    def search_qdrant(self, query: str, limit: int = 20) -> List[Dict]:
        ...
        try:
            ...
            print(f"Collection name: {self.valves.QDRANT_COLLECTION}")
