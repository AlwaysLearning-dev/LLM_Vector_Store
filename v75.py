...
class Pipeline:
    ...
    def extract_search_terms(self, query: str) -> str:
        """Extract raw search string (for embedding) after 'search_qdrant:' or fallback."""
        # If the user explicitly typed "search_qdrant:", parse after that
...
"""
    ...
    def pipe(self, prompt: str = None, **kwargs) -> Generator[str, None, None]:
        """Process input and return streaming results."""
        query = prompt or kwargs.get('user_message', '')
        if not query:
            return

        try:
            # Extract search text from the prompt
            search_text = self.extract_search_terms(query)
            
            # Always do a Qdrant vector search for context
    ...
    def run(self, prompt: str, **kwargs) -> List[Dict[str, Any]]:
        """Run pipeline and capture results in a list of dicts."""
